...
@section('content')
<h2 class="content-heading">Detail Pemesanan</h2>

<div class="block">
    <div class="block-header block-header-default">
        <h3 class="block-title">#{{ $pesan->id }}</h3>
    ...
    <div class="block-content">
        ...
        <div class="row my-20">
            ...
            <div class="col-6 text-right">
                <p class="h3">{{ $pesan->delivery->nama }}</p>
                <address>
                    {{ $pesan->delivery->alamat }}<br>
                    {{ $pesan->delivery->jumlah }}<br>
                    {{ $pesan->delivery->no_hp }}<br>
                    {{ $pesan->delivery->warna }}<br>
                    {{ $pesan->delivery->ukuran }}<br>
                    {{ $pesan->delivery->bahan }}<br>
                    {{ $pesan->delivery->jumlah }}<br>
                    {{ $pesan->delivery->gambar }}<br>
                    <a href="mailto:{{ $pesan->delivery->email }}">{{ $pesan->delivery->email }}</a>
                </address>
            </div>
            <!-- END Client Info -->
        </div>
        ...
        <div class="table-responsive push">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                @foreach($pesan->detail as $key => $detail)
                <tr>
                ...
                <tr class="table-warning">
                    ...
                    <td class="font-w700 text-right">@rupiah($pesan->total_harga)</td>
                </tr>
                </tbody>
            </table>
        </div>
